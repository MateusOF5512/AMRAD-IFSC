...
export function ComparisonROIChart({ data }: ComparisonROIChartProps) {
  ...
  if (!data || data.length === 0) {
    return (
      <div className="flex items-center justify-center h-80 bg-gray-50 rounded-lg border border-gray-200">
        <p className="text-gray-500 text-sm">{t('experiments.charts.comparativeRoi.noData')}</p>
      </div>
    )
  }
  ...
  return (
    <div className="w-full bg-white rounded-lg p-4">
      <h4 className="text-sm font-bold text-gray-700 mb-4">{t('experiments.charts.comparativeRoi.title')}</h4>
