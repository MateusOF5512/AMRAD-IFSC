...
interface ComparisonHUChartProps {
...
export function ComparisonHUChart({ data }: ComparisonHUChartProps) {
  ...
  const chartData = Array.from(infillTypes).map((infillType) => {
    ...
    data.forEach((exp, expIndex) => {
      ...
      if (infillData && infillData.hu_mean !== null && infillData.hu_mean !== undefined) {
        entry[`exp_${expIndex}`] = Number(infillData.hu_mean)
    ...
    <div className="w-full bg-surface rounded-lg p-4">
      ...
      <ResponsiveContainer width="100%" height={350}>
        <BarChart data={chartData} margin={{ top: 5, right: 30, left: 0, bottom: 60 }}>
          ...
          />
          <YAxis
            label={{ value: t('experiments.charts.comparativeHu.yAxisLabel'), angle: -90, position: 'insideLeft', fill: '#6b7280' }}
            stroke="#9ca3af"
            style={{ fontSize: '12px' }}
          />
          <Tooltip
            ...
            formatter={(value: any) => [Number(value).toFixed(2), t('experiments.charts.comparativeHu.tooltipLabel')]}
          />
